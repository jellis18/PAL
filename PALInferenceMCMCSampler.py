...
class PTPost(object):
    """
    Wrapper for posterior used with the MCMC code. This needs
    to be done so that we can use the multiprocessing module.
    ...
    def __init__(self, runState):
        ...
        @param runState: Run State Object that contains likelihood and prior functions

        """

        self._logl = runState.lnlikeFunc
        self._logp = runState.lnpriorFunc

    def __call__(self, params):
        """
        Returns ``lnpost(x)``, ``lnlike(x)``, where
...
# MCMC function
def MCMCSample(psr, runState, pardict):
    """
    ...
    if runState.nthreads > 1:
        pool = multi.Pool(runState.nthreads)
    else: 
        pool = None


    # get initial parameters from pardict
    p = np.array([pardict[key].value for key in pardict])

    
    ### compute initial log likelihoods ###

    # function pointer from PTPost class
    fn = PTPost(runState.lnlikeFunc, runState.lnpriorFunc)

    # inverse temperature
    betas = 1/runState.temperature

    if pool is None:
        results = list(map(fn, [(p[i, :], betas[i])\
                               for i in range(runState.ntemps)]))
    else:
        results = list(self.pool.map(fn, [(p[i, :], betas[i])\
...
class PTSampler(object):
    ...
    def _get_jump(self, p, beta=1):
        ...
        return self.propJump(p,self.iterations,beta=1)
